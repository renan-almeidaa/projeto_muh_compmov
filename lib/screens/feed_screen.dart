...
class _FeedScreenState extends State<FeedScreen> {
  ...
  Future<List<Widget>> _getAllPublications(UserModel model) async {
    ...
    for (DocumentSnapshot ds in await model.getUsers()) {
      // Pega os usuários
      // Monta o mapa com as informações do usuário
      Map<String, String> userInfo = {
        "userId": ds.documentID,
        "userName": ds.data["name"] + " " + ds.data["lastname"],
        "userImage": ds.data["image"],
      };

      List<DocumentSnapshot> pubListDS =
          await model.getUserPublication(ds.documentID);
      for (DocumentSnapshot pub_ds in pubListDS) {
        // Pega as publicações do usuário
